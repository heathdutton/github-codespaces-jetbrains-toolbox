...
class GhCli(
     ...
     * Note: gh CLI may output warnings about unavailable codespaces but still return valid config.
     */
    fun getSshConfig(): Result<String> {
        return try {
            val result = execute("codespace", "ssh", "--config")
            val output = result.outputString()

            // Filter out warning lines (lines starting with "skipping")
            // and keep only valid SSH config
            val configLines = output.lines().filter { line ->
                !line.startsWith("skipping ") && line.isNotBlank()
            }

            if (configLines.any { it.trim().startsWith("Host ") }) {
                // We have valid SSH config
                Result.success(configLines.joinToString("\n"))
            } else if (result.exitValue == 0) {
                Result.success(output)
            } else {
                Result.failure(GhCliException("Failed to get SSH config: $output"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    fun getSshHostForCodespace(codespaceName: String): Result<String> {
        return getSshConfig().map { config ->
            parseHostFromConfig(config, codespaceName)
        }
    }

    /**
     * Write SSH config and return the host for a codespace in one operation.
     * This is more efficient than calling writeSshConfig() and getSshHostForCodespace() separately.
     */
    fun writeSshConfigAndGetHost(codespaceName: String): Result<String> {
        return getSshConfig().map { config ->
            // Write the config
            val sshDir = java.io.File(System.getProperty("user.home"), ".ssh")
            sshDir.mkdirs()
    ...
    private fun parseHostFromConfig(config: String, codespaceName: String): String {
